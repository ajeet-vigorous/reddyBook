...
const AllCasinoLists = () => {
  ...
  const [multiplyModal, setMultiplyModal] = useState({ open: false, gameId: null });
  ...
  const handleLabel1 = (tableKey) => {
    const resolvedGame =
      tableKey === "all" ? Object.keys(CasinoList.allTables)[0] : "all";
...
console.log(userInfo?.data?.intCasinoMultiply, "ssssssssssssssssssss");
    ...
    <div className="w-full">
      ...
      <div className="flex overflow-x-auto ">
        {level2Keys.map((key) => (
          ...
          ${normalizeKey(gameName) === normalizeKey(key) ? "filter brightness-0 invert" : ""}`}
            />
            <span className="uppercase px-2 text-xs font-normal">{key}</span>
          </button>
        ))}
      </div>
            ...
            <img
              ...
              onClick={() => {
                if (localStorage.getItem("token")) {
                  ...
                  if (multiply > 1) {
                    setMultiplyModal({ open: true, gameId: game.id, value: multiply });
                  } else {
      ...
      {multiplyModal.open && (
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/50"
          onClick={() => setMultiplyModal({ open: false, gameId: null })}
        >
          ...
          >
            ...
            <div className="flex gap-2 p-3 bg-white">
              ...
              <button
                ...
                onClick={() => {
                  window.location.href = `/iframe-casino/${multiplyModal.gameId}`;
                }}
              >
                OK
              </button>
