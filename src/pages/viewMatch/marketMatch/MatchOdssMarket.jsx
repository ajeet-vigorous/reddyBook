      ...
      {Object.values(finalSocket).map((element, index) =>
        element.marketType === "Match Odds" && (
          <div className="" key={index}>
            <MatchDetailsHeaderSection marketType={"Match Odds"} minMax={{ min: 100, max: formatNumber(isMatchCoin?.max) }}>
              ...
              {element?.runners?.length > 0 && element.runners.map((elementtemp, index) => (
                <>
                  ...
                  {betSlipData?.oddsType === "Match Odds" && elementtemp?.selectionId === betSlipData?.selectionId && <PlaceBetMobile
                    openBets={openBets}
                    closeRow={closeRow}
