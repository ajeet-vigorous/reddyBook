...
const CashOutSystem = (props) => {
  ...
  const handleCashout = () => {
    ...
    const stake = parseFloat(cashoutData.hedgeAmount.toFixed(2));
    const odds = cashoutData.hedgeOdds;
    const betType = cashoutData.hedgeType === "back" ? "L" : "K";

    // Pre-calculate position effect for this bet
    const positionEffect = {};
    marketList.forEach((item) => {
      const id = item.selectionid || item.selectionId;
      if (id == cashoutData.selectionId) {
        positionEffect[id] =
          betType === "L" ? stake * (odds - 1) : -1 * stake * (odds - 1);
      } else {
        positionEffect[id] = betType === "L" ? -1 * stake : stake;
      }
    });
    ...
    handleBackOpen({
      data: marketList.find((t) => t.selectionid === cashoutData.selectionId),
      nameOther: marketList.map((item) => ({
        ...item,
        selectionId: item.selectionId || item.selectionid,
        selectionid: item.selectionid || item.selectionId,
      })),
      type: cashoutData.hedgeType === "back" ? "Yes" : "No",
      odds: odds,
  ...
  return (
    <button
      ...
      className={`text-sm text-white px-3 py-0.5 ml-1 rounded-[1px] ${
        isCashoutAvailable
